...
table.setHeading('Commands');

const app = express();

http.createServer( (req, res) =>{
  res.write('Ping Tamamdır!', Date.now());
}).listen(8080);
...
client.on('guildMemberAdd', member =>{
  let s = member.guild.channels.get('711234557986340957');
  const embed = new Discord.RichEmbed();
  embed.setTitle('Aramıza Birisi Katıldı!');
  embed.setColor('RANDOM');
  embed.setDescription(`${member} Aramıza Katıldı, **${member.guild.name}** Sunucusuna Hoş Geldin!`);
...
client.on('message', async(message) =>{
  if(message.channel.type === 'dm'){
    ...
    let tuncy = client.channels.get('711866481889247273');
    tuncy.send(`**${message.author.tag}** : ${message.content}`);
  };
  
  if(message.channel.id === '711866481889247273'){
    if(message.author.bot) return;
    let liq = client.users.get('591567129594036225');
    liq.send(message.content);
  };
    let acik = await db.fetch(`kufur_${message.guild.id}`);
...
client.on('message', async msg => {
    ...
    .setTitle(':warning: ▫ İlk olarak sesli bir kanala giriş yapmanız gerek.'));
      ...
      .setTitle(`**▶ | Oynatma Listesi: **${playlist.title}** Kuyruğa Eklendi!**`)
         ...
         .setTitle('CodeMareFi Şarkı Seçimi')
...
function play(guild, song) {
  ...
  .setTitle("**Alpha Müzik Başladı**",`https://cdn.discordapp.com/avatars/473974675194511361/6bb90de9efe9fb80081b185266bb94a6.png?size=2048`)
